...
class LogController extends Controller
{
    ...
    public function export(Request $request)
    {
        ...
        $start = $request->query('start_date');
        $end = $request->query('end_date');
        $format = strtoupper($request->query('format', 'EXCEL'));
        ...
        $data = $q->get()->map(function ($log) {
            return [
                ...
                'entity_id' => $log->entity_id,
                'details' => json_encode($log->metadata_json),
            ];
        });
        if ($format === 'CSV') {
            $filename = 'audit_logs_' . time() . '.csv';
            $headers = [
        ...
        return Excel::download(new GenericReportExport($data), 'audit_logs_' . time() . '.xlsx');
    }
}