...
class ExcelImportService
{
    ...
    {
        if (!empty($rowA6_2[12])) return 'Dalam Kelurahan/Kecamatan';
        if (!empty($rowA6_2[13])) return 'Luar Kecamatan';
        if (!empty($rowA6_2[14])) return 'Di Kota Lain';
        if (!empty($rowA6_2[15])) return 'Tidak Sekolah';
        if (!empty($rowA6_2[16])) return 'Tidak Ada Anggota Usia Wajib Belajar';
        return null;
    }
    ...
    {
        if (!empty($row[6]) || !empty($row[7])) return 'LE1_5';
        if (!empty($row[8]) || !empty($row[9])) return 'EQ1_5';
        if (!empty($row[10]) || !empty($row[11])) return 'GT1_5';
        return 'LE1_5'; // Default
    }

    private function mapLandLegalStatus(array $row): string
    {
        if (!empty($row[11])) return 'SHM';
        if (!empty($row[12])) return 'PERJANJIAN';
        if (!empty($row[13])) return 'LAINNYA';
        return 'TIDAK_TAHU';
    }

    private function mapWasteDisposal(array $row): string
    {
        if (!empty($row[3])) return 'PRIVATE_BIN';
        if (!empty($row[4])) return 'COMMUNAL';
        if (!empty($row[5])) return 'BURNT';
        if (!empty($row[6])) return 'OPENSPACE';
        if (!empty($row[7])) return 'WATERBODY';
        return 'PRIVATE_BIN';
    }

    private function mapElectricityPower(array $row): int
    {
        if (!empty($row[10])) return 450;
        if (!empty($row[11])) return 900;
        if (!empty($row[12])) return 1300;
        if (!empty($row[13])) return 2200;
        return 450; // Default
    }
}
