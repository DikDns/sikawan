...
export function InfrastructureMapDisplay({
    ...
    const { L } = useLeaflet();
    const page = usePage();
    const itemsFromProps = (page.props as any)?.items as
        | InfrastructureItem[]
        | undefined;
    const groupFromProps = (page.props as any)?.group as
        | { legend_icon?: string | null; legend_color_hex?: string | null }
        | undefined;
    ...
    const parsedFeatures = useMemo(() => {
        return features
            ...
            .map((f) => {
                try {
                    const geometry =
                        typeof f.geometry_json === 'string'
                            ? JSON.parse(f.geometry_json)
                            : f.geometry_json;
                    return { ...f, geometry };
                } catch {
                    return null;
    ...
    const initialShapes = useMemo(() => {
        ...
        parsedFeatures.forEach((f) => {
            ...
            if (allowedGeometryType === 'Point' && g.type === 'Point') {
                ...
                const latlngs = g.coordinates.map(
    ...
    const attachInfoUI = useCallback(
        (layer: any, label?: string) => {
            ...
            const itemDetail: InfrastructureItem | undefined = Array.isArray(
                itemsFromProps,
            )
        ...
        (
            ...
            if (changeType === 'initialized') {
                ...
                const processChunk = () => {
                    ...
                    for (; i < end; i++) {
                        ...
                        const id = (layer as any).__initialId as
                            | number
                            | undefined;
            ...
            if (changeType === 'created') {
                ...
                sourceGroup.eachLayer((layer: L.Layer) => {
                    ...
                    const gtype: 'Point' | 'LineString' | 'Polygon' =
                        allowedGeometryType;
